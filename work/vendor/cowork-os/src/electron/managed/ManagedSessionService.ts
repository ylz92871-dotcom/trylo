...
import { createMediaPlaybackUrl } from "../media/media-token-store";
...
import { ensureRoutineSchema } from "../routines/service";
...
export class ManagedSessionService {
  ...
  private readonly soloRuntime: ManagedSessionRuntimeService;
  ...
  constructor(
    ...
    private readonly options: {
      ...
      safeStorageResolver?: () => import("../utils/safe-storage").SafeStorageLike | null;
    } = {},
  ) {
    this.ensureGovernanceSchema();
    ensureRoutineSchema(db);
    ...
    this.channelRepo =
      options.safeStorageResolver != null
        ? new ChannelRepository(db, options.safeStorageResolver)
    ...
    this.soloRuntime = new ManagedSessionRuntimeService(db, agentDaemon, {
      assertPermission: (workspaceId, check, principalId) =>
        this.assertWorkspacePermission(workspaceId, check, principalId),
  ...
  async createSession(input: ManagedSessionCreateInput): Promise<ManagedSession> {
    ...
    const mcpToolAccess = this.resolveMcpToolAccess(environment);
    if (mcpToolAccess.blockingError) throw new Error(mcpToolAccess.blockingError);
  ...
  async resumeSession(sessionId: string): Promise<{ resumed: boolean; session?: ManagedSession }> {
    ...
    const backingTeamRunId =
      session.backingTeamRunId || this.teamRunRepo.findByRootTaskId(session.backingTaskId)?.id;
    if (!backingTeamRunId) {
  ...
  bridgeTaskEventNotification(
    ...
    if (!session) return {};
    const backingTeamRunId =
      session.backingTeamRunId || this.teamRunRepo.findByRootTaskId(taskId)?.id;
    if (!backingTeamRunId) {
  ...
  refreshSession(sessionId: string): ManagedSession | undefined {
    const session = this.managedSessionRepo.findById(sessionId);
    if (!session) return undefined;
    const discoveredTeamRun =
      !session.backingTeamRunId && session.backingTaskId
  ...
  private buildAgentConfig(
    environment: ManagedEnvironment,
    version: ManagedAgentVersion,
    mcpToolAccess = this.resolveMcpToolAccess(environment),
  ): AgentConfig {
