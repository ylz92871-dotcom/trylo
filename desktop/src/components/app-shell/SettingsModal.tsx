      ...
      <form className="settings-modal" onSubmit={onSave}>
        ...
        <div className="settings-modal__body">
          ...
          <fieldset className="settings-modal__group">
            ...
            {/* P2 (spec §3.2 / §4.4): the unified permission
                level replaces the legacy `chat | plan | agent`
                select. The new level is the single source of
                truth for the picker in the composer; the
                legacy `permissionMode` is preserved on the
                shape so an existing user's value still loads
                (migrated at save time, not destructive). */}
            <label className="settings-modal__field">
          ...
          {/* ── Companion (desktop pet, spec §6.5) ───────────── */}
          <fieldset className="settings-modal__group">
            ...
            {/* Real state (audit §4.2 PET-P0-1): a launch that failed is
                VISIBLE here, with a fixed reason instead of silence. */}
          ...
          {/* ── Work 工具 (Trylo Tool Platform, audit P0-A §3.3) ── */}
          {props.toolPlatform ? <WorkToolsSettings platform={props.toolPlatform} /> : null}

          {/* ── Work 工具面 (Trylo Tool Platform) ───
              2026-09-06: 桌面控制已并入办公基底，不再有独立开关；
              这里只剩互斥的面选择（浏览器调试 / CAD）。 */}
          <fieldset className="settings-modal__group">
            ...
            {/* ── CAD/EDA 适配器 (TRYLO-CAD-EDA-TOOL-ADAPTER §8) ── */}
            <label className="settings-modal__field settings-modal__field--inline">
              <input
                type="checkbox"
          ...
          {/* ── Remote access (migration spec §8.1) ──────────── */}
          <fieldset className="settings-modal__group">
            <legend className="settings-modal__legend">
              ...
              <button
                ...
                onClick={() => {
                  // 2026-08-29: the primary on/off switch lives
                  // in the TopBar (Radio · 远程 button). The
                  // mirror checkbox here is kept so the user can
                  // also flip it from the settings page.
            ...
            {/* Real gateway state (spec §8.1.4): running / port / URL and
                on-demand pairing QR. */}
          ...
          {/* ── Task learning (Hermes) — Work surface ──────────── */}
          {/* TRYLO-DUAL-SURFACE-SPEC §2.4: this is NOT a User Learning setting.
              It only toggles whether Work turns mirror sessions into Hermes and
              may propose Skills. Code and User Learning are unaffected. */}
          <fieldset className="settings-modal__group">
